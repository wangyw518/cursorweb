...
1. **箱子**：矮箱（`lowHeight` 36）单跳能过；高箱（`highHeight` 78）只跳不稳，冲刺或冲+跳才稳（`jumpVy` 480 / `dashHopVy` 620）。
2. **开局**：`firstHazardX` 750，约 2.5s 才到第一危险；前 2 秒 `quietMs` 无残影，quiet 结束不会立刻撞死。
...
3. 约 **1.5 秒**（`ghostDelayMs`）后，半透明残影（玩家色、alpha 0.45、1px `#A8F7FA` 描边、最多 2 帧拖影）会沿**刚才那一下的同一物理轨迹**出现在「若只做了那一下」的当前位置。死亡闪黄描边（`#FFE66D`）。
